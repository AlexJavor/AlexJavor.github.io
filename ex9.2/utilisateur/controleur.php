...
function creer(){
	...
	if ( !isset ($_POST['mail']) ) {
		...
		if ($erreurs == null AND ($_POST["password"] === $_POST["confirm_password"]) AND doubleuser($_POST["mail"])){
			$donnees = $_POST;
...
function afficherFormulaire($mode, $donnees, $erreurs){
    ...
    $erreurConfirmPassword = $erreurs['confirm_password'];
    ...
    if (doubleuser($mail)){
        ...
        $dosusers= "*This user already exists";
    }
    
	$corps = <<<EOT
<form id="creation-form" name="creation-form" method="post" action="controleur.php?action=$action">

<label for="mail" style="text-decoration: underline;">Mail___________</label>
<input id="mail" type="email" name="mail" value="$mail" required aria-required="true" />
$dosusers
<p class="erreur">$erreurMail</p>

<label for="password" style="text-decoration: underline;">Password_______</label>
<input id="password" type="password" name="password" value="$password" required aria-required="true" />
<p class="erreur">$erreurPassword</p>

<label for="confirm_password" style="text-decoration: underline;">Confirm Password</label>
<input id="confirm_password" type="password" name="confirm_password" value="$confirm_password" oncopy="return false" onpaste="return false" required aria-required="true" />
$pwmatch
<p class="erreur">$erreurPassword</p>
...
http://votresite.com/utilisateur/controleur.php?action=valider&cle='.urlencode($cle).'
---------------
