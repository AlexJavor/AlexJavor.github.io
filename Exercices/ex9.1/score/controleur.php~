...
function lister(){
	...
	while($r = $result->fetch_assoc()) {
	   	$corps .= "<li>";
		$corps .= $r['id'].", ".$r['valeur'];
        if ( isset($_SESSION['id']) && $_SESSION['id'] == $r['idMail']){
            // liens 
    ...
    //alerte
    $message = "";
    $corps .= "<script type='text/javascript'>function alertFunction(idE){var=confirm('Voulez-vous vraiment supprimer cet enregistrement ?'); if(r==true){var lien='controleur.php?action=supprimer&id='+idE;location.replace(lien);}}</script>";
